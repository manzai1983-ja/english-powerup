...
ok('277文すべてカードになる', items.length === 277, 'got ' + items.length);
ok('76回ぶんの節ができる', secs.length === 76, 'got ' + secs.length);
ok('件数表示が入る', /277 文 \/ 76 回/.test(b.ids.totalN.textContent), b.ids.totalN.textContent);
ok('回ジャンプの選択肢がある', b.ids.jump.childNodes.length === 77, 'got ' + b.ids.jump.childNodes.length);
...
console.log('=== 記録 ===');
{
  ...
  const saved = JSON.parse(t.store['english-powerup-v1'] || '{}');
...
}
{
  ...
  const t = boot({ store: { 'english-powerup-v1': JSON.stringify({ [id0]: { r: 'ok', t: 1 } }) } });
  const c = cardFor(t, 0);
...
{
  /* 表示と読み上げが違う項目は語クリックを切る */
  const t = boot({});
  const idx = t.sandbox.DATA.findIndex(d => d.en !== d.say);
  ok('表示と読み上げが違う項目がある', idx >= 0, '' + idx);
  const c = cardFor(t, idx);
  ok('その項目は no-seek になる', /no-seek/.test(c.el.className), c.el.className);
  ok('その項目には語クリックを付けない', !c.en._handlers.click);
  c.speak._handlers.click();
  ok('それでも聞くは動く', t.spoken[t.spoken.length - 1].text === t.sandbox.DATA[idx].say,
     t.spoken[t.spoken.length - 1].text);
}
...
console.log('=== 日本語訳 ===');
{
  ...
  ok('全文に訳が付く', withJa === 277, 'got ' + withJa);
